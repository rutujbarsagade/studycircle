...
<section class="videos-container">
   ...
   <div class="box-container">
      ...
      <?php
         $select_content = $conn->prepare("SELECT * FROM `content2` WHERE playlist_id = ? AND status = ? ORDER BY date DESC");
         $select_content->execute([$get_id, 'active']);
         if($select_content->rowCount() > 0){
            while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){  
      ?>
    <a href="#" class="box" onclick="openInNewTab('watch_notes.php?get_id=<?= $fetch_content['id']; ?>'); return false;">
    <i class="fas fa-play"></i>
    <img src="uploaded_files2/<?= $fetch_content['thumb']; ?>" alt="">
